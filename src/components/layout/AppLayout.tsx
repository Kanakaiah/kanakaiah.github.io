        ...
        <div className="flex lg:flex-col w-full justify-between lg:justify-start lg:gap-3">
          {navLinks.map(link => (
            ...
            >
              {({ isActive }) => (
                <>
                  <link.icon className={`w-5 h-5 lg:w-5 lg:h-5 lg:mr-3 ${isActive ? 'text-white lg:text-accent-light' : ''}`} />
                  <span className={`text-[10px] lg:text-sm font-bold mt-1 lg:mt-0 ${isActive ? 'text-white lg:text-accent-light' : ''}`}>
                    {link.label}
                  </span>
                  {/* Desktop active indicator pill */}
                  <div className={`hidden lg:block absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-8 bg-accent rounded-r-full transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-0'}`} />
                </>
              )}
            </NavLink>
          ))}
